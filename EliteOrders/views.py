...
def analytics():
    ...
    # Calculate sales from last week
    one_week_ago = datetime.datetime.now().date() - datetime.timedelta(days=7)
    last_week_orders = Order.query.with_entities(Order.total).filter(Order.date >= one_week_ago).all()
    last_week_sales = sum(order.total for order in last_week_orders)
    ...
    figures =[total_items,number_of_sales,format(last_week_sales,".2f"),format(total_discount_amount,".2f")]
